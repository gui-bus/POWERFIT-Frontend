...
const DIFFICULTIES = ["Iniciante", "Intermediário", "Avançado", "Elite"];

export function UpsertExerciseDialog({
  exercise,
  trigger,
}: UpsertExerciseDialogProps) {
  ...
  const onSubmit = async (values: ExerciseFormValues) => {
    ...
    try {
      ...
      if (response.status === 201 || response.status === 204) {
        toast.success(
          isEditing ? "Exercício atualizado!" : "Exercício criado!",
        );
        setOpen(false);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {trigger || (
          <Button className="rounded-2xl bg-primary text-white font-black uppercase tracking-widest text-[10px] h-12 px-8 shadow-lg shadow-primary/20 hover:scale-105 transition-all active:scale-95 w-full max-w-xs">
                  ...
                  <FormItem className="w-full">
                    ...
                    <FormControl>
                      <Input
                        ...
                        className="h-12 bg-muted/30 border-border/50 rounded-2xl font-bold uppercase italic"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="md:col-span-2">
                  ...
                  >
                    <FormControl>
                      <SelectTrigger className="h-12 bg-muted/30 border-border/50 rounded-2xl font-bold uppercase italic w-full">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      {...field}
                      className="h-12 bg-muted/30 border-border/50 rounded-2xl font-medium italic text-xs"
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      {...field}
                      className="min-h-32 bg-muted/30 border-border/50 rounded-2xl font-medium italic text-xs resize-none"
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
